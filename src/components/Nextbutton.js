...
const Nextbutton = styled.button`
  cursor: pointer;
      ...
      background-color: ${(props) => props.color === '#4a64f5'? '#4a64f5': '#efefef'};
          font-size: 17px;
          border: none;
          height: 60px;
          width: 100%;
          max-width: 520px;
          color : ${(props) => props.color === '#4a64f5'? '#ffffff': '#b7b7b7'};
          font-weight : 100;
`
...
export default function NextButtonComponent({text, color}){ 
  const [click, setClick ] = useState(null); 

  return(
    <Nextbutton onClick = {() => setClick(!click)} click = {click} color={color} className='Bold'>
      {text}
    </Nextbutton>
  )
}
