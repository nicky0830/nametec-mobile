	...
	if(adn_send<adn_status){
		...
		for(var k in adn_param){if(adn_param.hasOwnProperty(k)){
			for(var p in adn_param[k]){if(adn_param[k].hasOwnProperty(p)){
				...
				if (adn_param[k][p]["ut"] == 'Item' || adn_param[k][p]["ut"] == 'Cart') {

					var hostType = adn_param[k][p]['mall'] ? adn_param[k][p]['mall'] : '';
					var addonUrl = '//kn.acrosspf.com';
					var solutionUrl = '';

					console.log('hostType::: ' + hostType);
					if (hostType == 'cafe24') {
						solutionUrl = addonUrl + '/pay/npay_cafe24.ad?ui=' + adn_param[k][p]['ui'];
					} else if (hostType == 'makeshop') {
						solutionUrl = addonUrl + '/pay/npay_makeshop.ad?ui=' + adn_param[k][p]['ui'];
					} else if (hostType == 'godomall') {
						solutionUrl = addonUrl + '/pay/npay_godomall.ad?ui=' + adn_param[k][p]['ui'];
					} else if (hostType == 'sixshop'){
						solutionUrl = addonUrl + '/pay/npay_sixshop.ad?ui=' + adn_param[k][p]['ui'];
					} else if (hostType == 'imweb'){
						solutionUrl = addonUrl + '/pay/npay_imweb.ad?ui=' + adn_param[k][p]['ui'];
					} else if(hostType == 'self'){
						solutionUrl = addonUrl + '/pay/npay_self.ad?ui=' + adn_param[k][p]['ui'];
					}

					if(solutionUrl){
						var scriptType = adn_param[k][p]["ut"];
						var c=document.createElement("script");
						c.type="text/javascript";
						c.charset='UTF-8';
						c.defer=true;
